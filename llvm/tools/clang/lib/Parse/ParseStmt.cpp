...
///       forall-statement: modified version of [C99 6.8.5.3]
...
StmtResult Parser::ParseForAllStatement(SourceLocation *TrailingElseLoc) {
  ...
  if (Tok.is(tok::semi)) {  // for (;
    ...
    FirstPart = Actions.ActOnCXXForAllRangeIdentifier(getCurScope(), Loc, Name,
						      attrs, attrs.Range.getEnd());
    ForRange = true;
  } else if (isForInitDeclaration()) {  // for (int X = 4;
    // Parse declaration, which eats the ';'.
  ...
  // Parse the second part of the for specifier.
  getCurScope()->AddFlags(Scope::BreakScope | Scope::ContinueScope);
  if (!ForRange && !SecondPart.isInvalid()) {
    // Parse the second part of the for specifier.
    if (Tok.is(tok::semi)) {  // for (...;;
      // no second part.
    } else if (Tok.is(tok::r_paren)) {
  ...
  return Actions.ActOnForAllStmt(ForAllLoc, T.getOpenLocation(), FirstPart.get(),
                              SecondPart, ThirdPart, T.getCloseLocation(),
                              Body.get());
}
