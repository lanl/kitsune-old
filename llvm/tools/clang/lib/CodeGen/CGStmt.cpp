...
#include "clang/AST/StmtNodes.inc"
  ...
  case Stmt::ForAllStmtClass:   EmitForAllStmt(cast<ForAllStmt>(*S));     break;    
  ...
  case Stmt::CXXForAllRangeStmtClass:
    EmitCXXForAllRangeStmt(cast<CXXForAllRangeStmt>(*S));
    break;    
...
void CodeGenFunction::EmitAttributedStmt(const AttributedStmt &S) {
  ...
  case Stmt::CXXForRangeStmtClass:
    ...
    break;
  case Stmt::CXXForAllRangeStmtClass:
    EmitCXXForAllRangeStmt(cast<CXXForAllRangeStmt>(*SubStmt), S.getAttrs());
    break;    
  case Stmt::CilkForStmtClass:
...
// the parallel form.
void
CodeGenFunction::EmitForAllStmt(const ForAllStmt &S,
...
void CodeGenFunction::EmitCXXForAllRangeStmt(const CXXForAllRangeStmt &S,
  ...
  // Evaluate the first pieces before the loop begins.
  EmitStmt(S.getRangeStmt());
  EmitStmt(S.getBeginStmt());
  EmitStmt(S.getEndStmt());
  ...
  // If there's an increment, the continue scope will be overwritten
  // later.
  llvm::BasicBlock *CondBlock = createBasicBlock("forall.range.cond");
  EmitBlock(CondBlock);
  ...
  // to bool, is true.
  llvm::Value *BoolCondVal = EvaluateExprAsBool(S.getCond());
  Builder.CreateCondBr(BoolCondVal, ForAllBody, ExitBlock,
		       createProfileWeightsForLoop(S.getCond(), getProfileCount(S.getBody())));
